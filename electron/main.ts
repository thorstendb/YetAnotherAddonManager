...
ipcMain.handle(IPC_CHANNELS.SAVE_UI_SETTINGS, async (_event, settings: { logHeight?: number; panelWidths?: number[]; fontScale?: number; fontFamily?: string; skipCleanupConfirm?: boolean; autoUpdateOnStart?: boolean; showDependencies?: boolean }) => {
  const config = loadConfig();
  ...
  if (settings.showDependencies !== undefined) config.showDependencies = settings.showDependencies;
  saveConfig(config);
  return config;
});
